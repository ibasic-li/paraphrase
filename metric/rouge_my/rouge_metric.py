...
def rouge_metric(all_hypothesis, all_references, aggregator_list):
    ...
    for aggregator in aggregator_list:
        ...
        for metric, results in sorted(scores.items(), key=lambda x: x[0]):
            if apply_avg or apply_best:  # value is a type of list as we evaluate each summary vs each reference
...
def metric_from_file(hypothesis_path, references_path, size):
    ...
    result = rouge_metric(all_hypothesis[:size], all_references[:size], aggregator_list)
    end_time = time.time()
